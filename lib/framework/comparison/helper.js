...
var compareDomains = function compareDomains(base, other) {
  return compareSets(base.domains, other.domains, function () {
    return other.domains.has(_constants.domains.GENERIC);
  });
};
...
var compareTargets = function compareTargets(base, other) {
  return compareSets(base.targets, other.targets, function () {
    return other.targets.has(_constants.targets.GENERAL);
  });
};
...
var calculateScore = function calculateScore(comparison) {
  ...
  var domainScore = weights.domains * Math.max.apply(Math, (0, _toConsumableArray2["default"])(Object.values(comparison.domains)));
  var targetScore = weights.targets * Math.max.apply(Math, (0, _toConsumableArray2["default"])(Object.values(comparison.targets)));
  var featureScore = weights.features * Object.values(comparison.features).reduce(function (sum, v) {
    return sum + v;
  });
  var featureCnt = Object.keys(comparison.features).length;
